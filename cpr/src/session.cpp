...
class Session::Impl {
  public:
    ...
    static void SetSSLCertificate(const std::string& certFile) { sSSLCertificate = certFile; }
...
std::string Session::Impl::sSSLCertificate = "";
...
Response Session::Impl::makeRequest(CURL* curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_CAINFO, sSSLCertificate.c_str());
...
void Session::SetSSLCertificate(const std::string& certFile) { Session::Impl::SetSSLCertificate(certFile); }
